...
function apollo_jetpack_setup() {
	...
	add_theme_support( 'featured-content', array(
		'filter'      => 'apollo_get_featured_posts',
		'description' => esc_html__( 'The featured content section displays on the index page bellow the header.', 'apollo-by-gravityux' ),
		'max_posts'   => 6,
...
function apollo_infinite_scroll_render() {
	if ( class_exists( 'WooCommerce' ) && ( is_shop() || is_product_taxonomy() || is_product_category() || is_product_tag() ) ) {
		apollo_woocommerce_product_columns_wrapper();
	...
	if ( class_exists( 'WooCommerce' ) && ( is_shop() || is_product_taxonomy() || is_product_category() || is_product_tag() ) ) {
		...
		apollo_woocommerce_product_columns_wrapper_close();
	}
}
 ...
 * Featured Posts.
 */
function apollo_has_multiple_featured_posts() {
	$featured_posts = apply_filters( 'apollo_get_featured_posts', array() );
	if ( is_array( $featured_posts ) && 1 < count( $featured_posts ) ) {
		return true;
	}
	return false;
}
function apollo_get_featured_posts() {
	return apply_filters( 'apollo_get_featured_posts', false );
}

/**
 * Return early if Social Menu is not available.
 */
function apollo_social_menu() {
 ...
 * Testimonials Title.
 */
function apollo_testimonials_title( $before = '', $after = '' ) {
	$jetpack_options = get_theme_mod( 'jetpack_testimonials' );
	$title = esc_html__( 'Testimonials', 'apollo-by-gravityux' );
 ...
 */
function apollo_testimonials_content( $before = '', $after = '' ) {
	$jetpack_options = get_theme_mod( 'jetpack_testimonials' );
 ...
 */
function apollo_testimonials_image() {
	$jetpack_options = get_theme_mod( 'jetpack_testimonials' );
 ...
 */
function apollo_author_bio_avatar_size() {
	return 60;
}
add_filter( 'jetpack_author_bio_avatar_size', 'apollo_author_bio_avatar_size' );

/**
 * Load Jetpack scripts.
 */
function apollo_jetpack_scripts() {
	if ( is_home() && ! is_paged() && false !== apollo_get_featured_posts() ) {
		wp_register_script( 'jquery-flexslider', get_template_directory_uri() . '/js/jquery.flexslider.js', array( 'jquery' ), '2.6.0', true );

		wp_enqueue_script( 'apollo-featured-content', get_template_directory_uri() . '/js/featured-content.js', array( 'jquery-flexslider' ), '20160411', true );
	}
}
add_action( 'wp_enqueue_scripts', 'apollo_jetpack_scripts' );

/**
 * Hide Featured Image on single posts view outside of the loop.
 */
function apollo_jetpack_featured_image_post() {
 ...
 */
function apollo_has_post_thumbnail( $post = null ) {
 ...
 */
function apollo_get_attachment_image_src( $post_id, $post_thumbnail_id, $size ) {
